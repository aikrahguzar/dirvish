...
(defun dirvish-subtree--move-to-file (file depth)
  ...
  (let ((case-fold-search nil)
    ...
    (while (and (not stop)
                (setq end (search-forward file nil 'move)))
      (or (setq stop (and (eq depth (dirvish-subtree--depth))
                          (eq (match-beginning 0) (dired-move-to-filename))
                          (eq (dired-move-to-end-of-filename) end)))
          (goto-char end)))
    stop))
